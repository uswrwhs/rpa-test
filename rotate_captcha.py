...
def discern(inner_image_brg, outer_image_brg, result_img=None, pic_circle_radius=None, isSingle=False):
    ...
    all_deviation = []
    pic_circle_radius = pic_circle_radius if pic_circle_radius else (inner_image_brg.shape[0] // 2)
    total = 180 if isSingle else 360
    for result in range(0, total, 1 if isSingle else 2):
...
if __name__ == '__main__':
    ...
    result = discern('./publicPicture/inner.jpeg', './publicPicture/outer.jpeg', isSingle=True)
