...
logger.add(f'{LOG_PATH}/{formatted_time}.log', format="{time} {level} {message}", level="INFO")
    ...
    def start_userID(self, user_id):
        ...
        if res.status_code == 200:
            ...
            if res_result["msg"] == "Success" or res_result["msg"] == "success":
                ...
                return selenium_webdriver_su, selenium_address_su, user_id
        else:
...
def brushVideo(page_brush: ChromiumPage, brush_user_id):
    ...
    while True:
        page_brush.wait(5, 8)
        # 滑动验证
        validation(page_brush, brush_user_id)
...
def getLive_user_id(page_brush: ChromiumPage):
    ...
    # 将时间格式化为指定格式
    getLiveFans_currentTime = now.strftime("%Y-%m-%d=%H_%M_%S")
    file_name = f'{getLiveFans_currentTime}--{user_id_get}.txt'
